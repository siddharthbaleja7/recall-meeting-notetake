  ...
  return (
    <main className="share-page">
      <header className="share-header">
        ...
        <span className="shared-label">Shared meeting record</span>
      </header>
      ...
      <div className="share-grid">
        ...
        <section className="share-transcript"><div className="transcript-toolbar"><div><h2>Transcript</h2><span>{meeting.transcript.length} lines</span></div></div><div className="transcript-list">{meeting.transcript.map((line) => <article className={`transcript-line ${line.highlighted ? "highlighted" : ""}`} key={line.id}><span className="line-time">{line.timestamp}</span><div className="line-body"><div className="speaker-row"><span className="speaker-dot" />{line.speaker}</div><p>{line.text}</p></div></article>)}</div></section>
      </div>
      <footer className="share-footer">This meeting was shared from a Recall workspace.</footer>
    </main>
  );
}
